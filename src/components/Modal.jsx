...
import { Alert, Modal, StyleSheet, Text, Pressable, View, TextInput, TouchableOpacity, Image, ScrollView } from 'react-native';
import shotingTables from '../DB/shotingTables';
import { temperatureCharge, componentsWind, windSpeedObj, verticalTemperatureObj } from '../DB/WeatherTables';
import Cancel from '../image/cancel.png'
import AsyncStorage from '@react-native-async-storage/async-storage';
import MeteoData from './MeteoData';

const ModalBlock = ({ angle, heightFP, rangeСalculation, trajectory, nameChargePrimary }) => {
    ...
    React.useEffect(() => {
        ...
        pressure, airTemperature, directorateAngleWind, windSpeed, deviationInitialSpeed, chargeTemperature, heightWeatherPost
    ])
    ...
    const deviationInitialSpeedChargeCalculation = () => {
        try {
            ...
            const supportTemperature = temperatureCharge.filter(el => el.name === nameCharge.toLocaleLowerCase()).find(el => el.temperature >= (+chargeTemperature)).temperature
            const supportdV0 = temperatureCharge.filter(el => el.name === nameCharge.toLocaleLowerCase()).find(el => el.temperature >= (+chargeTemperature)).dV0
            const step = temperatureCharge.filter(el => el.name === nameCharge.toLocaleLowerCase()).find(el => el.temperature >= (+chargeTemperature)).step
            const dVoCharge = ((+chargeTemperature) - supportTemperature) * step + supportdV0
    ...
    const totalDeviationInitialSpeedCalculation = () => {
        const totalDeviationInitialSpeed = +deviationInitialSpeed + deviationInitialSpeedChargeCalculation()

        return totalDeviationInitialSpeed
    }
    /**рассчитываем dH */
    const deviationGroundPressureCalculation = () => {
        const deviationGroundPressure = (750 - (+pressure)) + ((heightWeatherPost - heightFP) / 10)
    ...
    const supportingRangeCalculation = () => {
        try {
            ...
            if (trajectory.toLocaleLowerCase() === 'н') {
                supportingRange = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range >= rangeСalculation).range
            } else {
                supportingRange = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range <= rangeСalculation).range
            }
    ...
    const returnHighEntranceInBulletin = () => {
        try {
            ...
            if (trajectory.toLocaleLowerCase() === 'н') {
                highEntranceInBulletin = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range >= rangeСalculation).Yb
            } else {
                highEntranceInBulletin = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range <= rangeСalculation).Yb
            }
            ...
            } else if (highEntranceInBulletin > 200 && highEntranceInBulletin <= 400) {
                highEntranceInBulletin = 400
            } else if (highEntranceInBulletin > 400 && highEntranceInBulletin <= 800) {
                highEntranceInBulletin = 800
            } else if (highEntranceInBulletin > 800 && highEntranceInBulletin <= 1200) {
                highEntranceInBulletin = 1200
            } else if (highEntranceInBulletin > 1200 && highEntranceInBulletin <= 1600) {
                highEntranceInBulletin = 1600
            } else if (highEntranceInBulletin > 1600 && highEntranceInBulletin <= 2000) {
                highEntranceInBulletin = 2000
            } else if (highEntranceInBulletin > 2000 && highEntranceInBulletin <= 2400) {
                highEntranceInBulletin = 2400
            } else if (highEntranceInBulletin > 2400 && highEntranceInBulletin <= 3000) {
                highEntranceInBulletin = 3000
            } else if (highEntranceInBulletin > 3000 && highEntranceInBulletin <= 4000) {
    ...
    const returnDeviationAirTemperature = () => {
        try {
            const deviationAirTemperature = verticalTemperatureObj[airTemperature][returnHighEntranceInBulletin()]
    ...
    const returnSpeedNediumWind = () => {
        try {
            const speedNediumWind = windSpeedObj[windSpeed][returnHighEntranceInBulletin()]
    ...
    const returnDataST = () => {
        try {
            ...
            if (trajectory.toLocaleLowerCase() === 'н') {
                z = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range >= supportingRangeCalculation()).Z
                dZw = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range >= supportingRangeCalculation()).dZw
                dXw = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range >= supportingRangeCalculation()).dXw
                dXh = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range >= supportingRangeCalculation()).dXh
                dXt = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range >= supportingRangeCalculation()).dXt
                dXv0 = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range >= supportingRangeCalculation()).dXv0
            } else {
                z = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range <= supportingRangeCalculation()).Z
                dZw = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range <= supportingRangeCalculation()).dZw
                dXw = shotingTables.filter(el => el.name === nameCharge.toLocaleLowerCase()).filter(el => el.trajectory === trajectory.toLocaleLowerCase()).find(el => el.range <= supportingRangeCalculation()).dXw
    ...
    const returnLinkingWinds = () => {
        try {
            ...
            if (returnSpeedNediumWind() > 20) {
                let Wx1 = componentsWind.filter(el => el.Aw === angleTransform).find(el => el.Vw === 20).Wx
                let Wz1 = componentsWind.filter(el => el.Aw === angleTransform).find(el => el.Vw === 20).Wz
                let Wx2 = componentsWind.filter(el => el.Aw === angleTransform).find(el => el.Vw === returnSpeedNediumWind() - 20).Wx
                let Wz2 = componentsWind.filter(el => el.Aw === angleTransform).find(el => el.Vw === returnSpeedNediumWind() - 20).Wz
                ...
                Wx = componentsWind.filter(el => el.Aw === angleTransform).find(el => el.Vw === returnSpeedNediumWind()).Wx
                Wz = componentsWind.filter(el => el.Aw === angleTransform).find(el => el.Vw === returnSpeedNediumWind()).Wz
    ...
    const totalAmendmentsCalculation = () => {
        const totalAmendmentInRange = Math.round((0.1 * returnDataST().dXw * returnLinkingWinds().Wx) + (0.1 * returnDataST().dXh * deviationGroundPressureCalculation()) + (0.1 * returnDataST().dXt * returnDeviationAirTemperature()) + (returnDataST().dXv0 * totalDeviationInitialSpeedCalculation()))
        const totalAmendmentInDirection = ((returnDataST().z + (0.1 * returnDataST().dZw * returnLinkingWinds().Wz)) * 0.01).toFixed(2)
                    ...
                    <View style={styles.centeredView}>
                        <View style={styles.modalView}>
                            ...
                            <View style={{
                                ...
                                >
                                    {`Δδ: ${replaceAngle(totalAmendmentsCalculation().totalAmendmentInDirection)}`}
                                </Text>
                            </View>
                            <Pressable
